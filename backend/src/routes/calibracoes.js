...
export function register(router) {
    ...
    router.post('/api/calibracoes', async (ctx) => {
        ...
        return transaction(() => {
            ...
            run(`UPDATE padroes SET data_ultima_calibracao=?, data_proxima_calibracao=?,
                     status=?, atualizado_em=datetime('now') WHERE id = ?`,
                [b.data_calibracao, proxima, novoStatus, b.padrao_id]);
            registrarAuditoria(ctx, 'CALIBRACAO', 'calibracoes', r.lastInsertRowid,
                `Calibração registrada para ${padrao.codigo_interno} (${b.resultado || 'sem resultado'})`);
            ctx.status = 201;
            return get(`SELECT * FROM calibracoes WHERE id = ?`, [r.lastInsertRowid]);
        });
    });
    ...
    router.post('/api/checagens', async (ctx) => {
        ...
        return transaction(() => {
            ...
            run(`UPDATE padroes SET data_ultima_checagem=?, data_proxima_checagem=?,
                     atualizado_em=datetime('now') WHERE id = ?`,
                [b.data_checagem, proxima, b.padrao_id]);
            registrarAuditoria(ctx, 'CHECAGEM', 'checagens', r.lastInsertRowid,
                `Checagem registrada para ${padrao.codigo_interno}`);
            ctx.status = 201;
            return get(`SELECT * FROM checagens WHERE id = ?`, [r.lastInsertRowid]);
        });
    });
    ...
    router.get('/api/vencimentos', async (ctx) => {
        ...
        for (const l of linhas) {
            if (l.dias < 0) cat.vencidos.push(l);
            else if (l.dias <= 7) cat.em_7.push(l);
            else if (l.dias <= 15) cat.em_15.push(l);
            else if (l.dias <= 30) cat.em_30.push(l);
    ...
    router.get('/api/alertas', async (ctx) => {
        ...
        return {
            vencidos: cont.vencidos || 0, em_7: cont.em_7 || 0,
            em_15: cont.em_15 || 0, em_30: cont.em_30 || 0,
            total: (cont.vencidos || 0) + (cont.em_7 || 0) + (cont.em_15 || 0) + (cont.em_30 || 0),
        };
    });
