...
export function register(router) {
    router.get('/api/usuarios', async (ctx) => {
        ...
        return await all(`SELECT id, nome, email, perfil, ativo, telefone, cargo, twofa_ativo, ultimo_login, criado_em
    ...
    router.post('/api/usuarios', async (ctx) => {
        ...
        await registrarAuditoria(ctx, 'CRIAR', 'usuarios', r.lastInsertRowid, `Usuário ${email} (${b.perfil}) criado`);
        ctx.status = 201;
        return await get(`SELECT id, nome, email, perfil, ativo FROM usuarios WHERE id = ?`, [r.lastInsertRowid]);
    });

    router.put('/api/usuarios/:id', async (ctx) => {
        await exigirPerfil(ctx, 'administrador');
        const u = await get(`SELECT * FROM usuarios WHERE id = ? AND excluido_em IS NULL`, [ctx.params.id]);
        ...
        await registrarAuditoria(ctx, 'EDITAR', 'usuarios', u.id, `Usuário ${u.email} editado`, u);
        return await get(`SELECT id, nome, email, perfil, ativo FROM usuarios WHERE id = ?`, [u.id]);
    });
    ...
    router.post('/api/usuarios/:id/senha', async (ctx) => {
        ...
        await run(`UPDATE usuarios SET senha_hash=?, tentativas_login=0, bloqueado_ate=NULL WHERE id = ?`,
            [hashSenha(senha_nova), u.id]);
        await registrarAuditoria(ctx, 'RESET_SENHA', 'usuarios', u.id, `Senha redefinida pelo administrador`);
        return { ok: true };
    });
    ...
    router.delete('/api/usuarios/:id', async (ctx) => {
        ...
        await run(`UPDATE usuarios SET excluido_em=datetime('now'), excluido_por=?, motivo_exclusao=?, ativo=0 WHERE id = ?`,
            [ctx.usuario.id, motivo || 'sem motivo', u.id]);
        await registrarAuditoria(ctx, 'EXCLUIR', 'usuarios', u.id, `Usuário ${u.email} desativado/excluído`, u, null);
        return { ok: true };
    });
